...
pub async fn start_device_code(client: &Client, tenant_id: &str, client_id: &str) -> Result<DeviceCodeInfo, String> {
    ...
    let resp = client.post(&url).form(&params).send().await.map_err(|e| format!("Erreur réseau : {e}"))?;
    ...
    if !resp.status().is_success() {
        ...
        return Err(format!("Erreur d'authentification {status} : {desc}"));
    }

    resp.json::<DeviceCodeInfo>().await.map_err(|e| format!("Réponse device code invalide : {e}"))
}
...
pub async fn poll_token(client: &Client, tenant_id: &str, client_id: &str, device_code: &str, interval: u64) -> Result<TokenSet, String> {
    ...
    loop {
        ...
        if Instant::now() > deadline {
            return Err("L'authentification a expiré (10 minutes).".into());
        }
        ...
        match json.get("error").and_then(|v| v.as_str()).unwrap_or("") {
            ...
            "expired_token" => return Err("Le code d'appareil a expiré. Relancez la connexion.".into()),
            other => {
                let desc = json.get("error_description").and_then(|v| v.as_str()).unwrap_or(&body);
                return Err(format!("Erreur d'authentification ({other}) : {desc}"));
            }
        }
    }
}
...
pub async fn refresh_access_token(client: &Client, tenant_id: &str, client_id: &str, refresh_token: &str) -> Result<TokenSet, String> {
    ...
    if !status.is_success() {
        let desc = json.get("error_description").and_then(|v| v.as_str()).unwrap_or(&body);
        return Err(format!("Impossible de rafraîchir la session ({}) : {}", status.as_u16(), desc));
    }

    let access_token = json.get("access_token").and_then(|v| v.as_str()).ok_or("Réponse de refresh sans access_token.")?;
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs();
