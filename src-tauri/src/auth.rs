    ...
    loop {
        ...
        match json.get("error").and_then(|v| v.as_str()) {
            Some("authorization_pending") => {
                eprintln!("poll_token: authorization_pending");
                continue;
            }
            Some("slow_down") => {
                eprintln!("poll_token: slow_down");
                sleep(Duration::from_secs(5)).await;
                continue;
            }
            Some("authorization_declined") => {
                eprintln!("poll_token: authorization_declined");
                return Err("User declined authorization".into());
            }
            Some("expired_token") => {
                eprintln!("poll_token: expired_token");
                return Err("Device code expired. Please retry.".into());
            }
            Some(e) => {
                let desc = json.get("error_description")
                    .and_then(|v| v.as_str())
                    .unwrap_or(e);
                eprintln!("poll_token error raw body: {}", body);
                return Err(format!("Auth error: {}", desc));
            }
            None => {
                eprintln!("poll_token unexpected body: {}", body);
