...
export default function PhoneUsersTab({ data, onActionDone }: Props) {
  ...
  async function handleUnassign() {
    ...
    try {
      ...
      setConfirm((c) => c ? { ...c, loading: false, result: { ok: true, msg: `Numéro dissocié de ${c.user.displayName}.` } } : null);
    } catch (e) {
        ...
        >
          <div style={{
            ...
            <h3 style={{ margin: "0 0 6px", fontSize: 16, fontWeight: 700, color: "var(--text-1)" }}>
              Désassigner le numéro
            </h3>
            <p style={{ margin: "0 0 4px", fontSize: 13, color: "var(--text-2)" }}>
              {confirm.user.displayName}
            </p>
            <p style={{ margin: "0 0 18px", fontSize: 13, color: "var(--text-3)", fontFamily: "monospace" }}>
              {confirm.user.phoneNumber}
            </p>
            <p style={{ margin: "0 0 18px", fontSize: 13, color: "var(--text-2)", lineHeight: 1.5 }}>
              Cette action supprimera l'affectation du numéro Teams et retirera la politique de routage voix de cet utilisateur.
            </p>

            {confirm.result && (
              <div style={{
                padding: "10px 14px", borderRadius: 6, marginBottom: 14,
                background: confirm.result.ok ? "var(--success-bg)" : "rgba(239,68,68,0.1)",
                border: `1px solid ${confirm.result.ok ? "rgba(16,185,129,0.3)" : "rgba(239,68,68,0.3)"}`,
                color: confirm.result.ok ? "var(--success)" : "#f87171",
                fontSize: 13,
              }}>
                {confirm.result.msg}
              </div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button className="btn" style={{ fontSize: 13, padding: "6px 16px" }} onClick={closeConfirm}>
                {confirm.result?.ok ? "Fermer & actualiser" : "Annuler"}
              </button>
              {!confirm.result?.ok && (
                <button
                  className="btn"
                ...
                >
                  {confirm.loading ? "En cours…" : "Confirmer la désassignation"}
                </button>
              )}
            </div>
          </div>
