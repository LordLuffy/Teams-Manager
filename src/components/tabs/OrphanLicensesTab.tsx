...
export default function OrphanLicensesTab({ data, freeNumbers, onActionDone }: Props) {
  ...
  async function handleAssign() {
    ...
    try {
      ...
      setAction({ loading: false, result: { ok: true, msg: `${selectedNumber} → ${modalUser.displayName}` } });
    } catch (e) {
  ...
  const columns: Column<OrphanLicense>[] = [
    ...
    },
    {
      ...
      render: (_, row) => (
        ...
        >
          {t("tabs.orphanLicenses.assignBtn")}
        </button>
      ),
    },
  ];
  ...
  for (const fn_ of freeNumbers) {
    const key = fn_.numberType || "Autre";
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, background: "var(--warning-bg)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 8, padding: "12px 16px", marginBottom: 18 }}>
        ...
        <p
          style={{ color: "var(--warning)", fontSize: 13, margin: 0, lineHeight: 1.5 }}
          dangerouslySetInnerHTML={{ __html: t("tabs.orphanLicenses.alertDesc") }}
        />
      </div>

      <DataTable<OrphanLicense> columns={columns} data={data} exportFilename="unlicensed_users.csv" />
      ...
      {modalUser && (
        ...
        >
          <div style={{ background: "var(--bg-primary)", border: "1px solid var(--border)", borderRadius: 12, padding: "24px 28px", width: 480, maxWidth: "calc(100vw - 40px)", maxHeight: "80vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
            <h3 style={{ margin: "0 0 6px", fontSize: 16, fontWeight: 700, color: "var(--text-1)" }}>
              {t("tabs.orphanLicenses.modalTitle")}
            </h3>
            ...
            {freeNumbers.length === 0 ? (
              <p style={{ color: "var(--warning)", fontSize: 13 }}>
                {t("tabs.orphanLicenses.noFreeNumbers")}
              </p>
            ) : (
                  ...
                  <div key={type}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {nums.map((fn_) => (
                        <label
                          key={fn_.number}
                          style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 6, cursor: "pointer", background: selectedNumber === fn_.number ? "rgba(96,165,250,0.12)" : "var(--bg-secondary)", border: `1px solid ${selectedNumber === fn_.number ? "rgba(96,165,250,0.4)" : "var(--border)"}`, transition: "background 0.15s, border-color 0.15s" }}
                        >
                          <input
                            type="radio"
                            name="phoneNumber"
                          ...
                          />
                          <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-1)", fontFamily: "monospace" }}>{fn_.number}</span>
                          {fn_.city && <span style={{ fontSize: 12, color: "var(--text-3)" }}>{fn_.city}</span>}
                        </label>
                      ))}
                    </div>
                  </div>
            ...
            {action.result && (
              <div style={{ padding: "10px 14px", borderRadius: 6, marginBottom: 14, background: action.result.ok ? "var(--success-bg)" : "rgba(239,68,68,0.1)", border: `1px solid ${action.result.ok ? "rgba(16,185,129,0.3)" : "rgba(239,68,68,0.3)"}`, color: action.result.ok ? "var(--success)" : "#f87171", fontSize: 13 }}>
                {action.result.msg}
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button className="btn" style={{ fontSize: 13, padding: "6px 16px" }} onClick={closeModal}>
                {action.result?.ok ? t("common.closeRefresh") : t("common.cancel")}
              </button>
              {!action.result?.ok && (
                <button
                ...
                >
                  {action.loading ? t("common.inProgress") : t("tabs.orphanLicenses.confirmAssign")}
                </button>
              )}
            </div>
          </div>
